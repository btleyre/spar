...
class Learner(nn.Module):
    def __init__(self, args, hid_dim = 128, weights = None):
        ...
        self.fclayer = nn.Sequential(nn.Linear(hid_dim, 1, bias=args.use_bias))
...
class Learner_RCF_MNIST(nn.Module):
    def __init__(self, args, weights = None):
        ...
        self.fc = nn.Linear(num_ftrs, 1, bias=args.use_bias)
...
class Learner_TMLR(nn.Module):
    def __init__(self, args, weights = None):
        super(Learner_TMLR, self).__init__()
        self.args = args

        # The hidden dimension is 512
        self.resnet34 = resnet34(spectral_normalization=False)
        self.fc = nn.Linear(512, 1, bias=args.use_bias)

    def reset_weights(self, weights):
        self.load_state_dict(deepcopy(weights))

    def forward(self, x, return_reps=False):
        reps = self.resnet34(x)
        output = self.fc(reps)
        if return_reps:
            return output, reps
        else:
    ...
    def forward_mixup(self, x1, x2, lam = None):
        reps1 = self.resnet34(x1)
        reps2 = self.resnet34(x2)

        # mixup feature
        reps = lam * reps1 + (1 - lam) * reps2
