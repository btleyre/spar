...
class Model(nn.Module):
    def __init__(self, args, weights):
        ...
        #resnet18_ms
        self.enc = ResNet18(num_classes=1, num_channels=8, use_bias=args.use_bias)
    ...
    def getDataLoaders(args, device):
        ...
        dataset_unlabeled = PovertyMapUnlabeledDataset(
            root_dir=os.path.join(args.data_dir, 'wilds'),
            download=True, **kwargs
        )
        # get all train data
        ...
        datasets_unlabeled = {}
        for split in dataset_unlabeled.split_dict:
            datasets_unlabeled[split] = dataset_unlabeled.get_subset(split, transform=transform)
        ...
        return train_loaders, tv_loaders, unlabeled_loaders
